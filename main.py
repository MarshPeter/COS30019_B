...
# Create the Graph with Longitude and Latitude
def create_sample_graph_geo(node_data, edges_data, flows_data):
    G = nx.Graph()

    # Will contain tuples in form (long, lat)
    locations_plot = {}
    ...
    # Assume flow in both directions is possible
    ...
    return G, locations_plot # Return graph and plotting coordinates
...
def main(nodes_csv, edges_csv, flows_csv):
    ...
    if st.button("CalculatePath"):
        if start_location == end_location:
            ...
            # Some algorithms output in different formats, this allows all the formaat to print in a consistent manner. 
...
def get_path(model, algorithm, hour, origin, destination):
    ...
    # returns result of respective algorithm that the user chose
    if algorithm == "BFS":
        solution = BFS(graph)
